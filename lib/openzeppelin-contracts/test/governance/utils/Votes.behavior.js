...
function shouldBehaveLikeVotes(accounts, tokens, { mode = 'blocknumber', fungible = true }) {
  ...
  describe('run votes workflow', function () {
    ...
    describe('getPastcollateral', function () {
      ...
      it('reverts if block number >= current block', async function () {
        ...
        await expectRevertCustomError(this.votes.getPastcollateral(timepoint), 'ERC5805FutureLookup', [
          timepoint,
          clock,
        ]);
      });
      ...
      it('returns 0 if there are no checkpoints', async function () {
        expect(await this.votes.getPastcollateral(0)).to.be.bignumber.equal('0');
      });
      ...
      it('returns the correct checkpointed total supply', async function () {
        ...
        expect(await this.votes.getPastcollateral(t0.timepoint - 1)).to.be.bignumber.equal('0');
        expect(await this.votes.getPastcollateral(t0.timepoint)).to.be.bignumber.equal(weight[0]);
        expect(await this.votes.getPastcollateral(t0.timepoint + 1)).to.be.bignumber.equal(weight[0]);
        expect(await this.votes.getPastcollateral(t1.timepoint)).to.be.bignumber.equal(weight[0].add(weight[1]));
        expect(await this.votes.getPastcollateral(t1.timepoint + 1)).to.be.bignumber.equal(weight[0].add(weight[1]));
        expect(await this.votes.getPastcollateral(t2.timepoint)).to.be.bignumber.equal(weight[0]);
        expect(await this.votes.getPastcollateral(t2.timepoint + 1)).to.be.bignumber.equal(weight[0]);
        expect(await this.votes.getPastcollateral(t3.timepoint)).to.be.bignumber.equal(weight[0].add(weight[2]));
        expect(await this.votes.getPastcollateral(t3.timepoint + 1)).to.be.bignumber.equal(weight[0].add(weight[2]));
        expect(await this.votes.getPastcollateral(t4.timepoint)).to.be.bignumber.equal(weight[2]);
        expect(await this.votes.getPastcollateral(t4.timepoint + 1)).to.be.bignumber.equal(weight[2]);
        expect(await this.votes.getPastcollateral(t5.timepoint)).to.be.bignumber.equal('0');
        await expectRevertCustomError(this.votes.getPastcollateral(t5.timepoint + 1), 'ERC5805FutureLookup', [

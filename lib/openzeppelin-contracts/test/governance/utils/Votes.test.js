  ...
  for (const [mode, artifact] of Object.entries(MODES)) {
    describe(`vote with ${mode}`, function () {
      ...
      it('starts with zero votes', async function () {
        expect(await this.votes.getcollateral()).to.be.bignumber.equal('0');
      });
      ...
      describe('performs voting operations', function () {
        ...
        it('reverts if block number >= current block', async function () {
          ...
          await expectRevertCustomError(this.votes.getPastcollateral(lastTxTimepoint + 1), 'ERC5805FutureLookup', [
            lastTxTimepoint + 1,
            clock,
          ]);
        });
        ...
        it('returns total amount of votes', async function () {
          const collateral = BNsum(...Object.values(amounts));
          expect(await this.votes.getcollateral()).to.be.bignumber.equal(collateral);
        });
      });
    });
  }

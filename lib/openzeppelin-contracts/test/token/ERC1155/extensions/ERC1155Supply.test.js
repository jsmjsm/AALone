...
contract('ERC1155Supply', function (accounts) {
  ...
  context('before mint', function () {
    ...
    it('collateral', async function () {
      expect(await this.token.methods['collateral(uint256)'](firstTokenId)).to.be.bignumber.equal('0');
      expect(await this.token.methods['collateral()']()).to.be.bignumber.equal('0');
    });
  });
  ...
  context('after mint', function () {
    context('single', function () {
      ...
      it('collateral', async function () {
        expect(await this.token.methods['collateral(uint256)'](firstTokenId)).to.be.bignumber.equal(firstTokenValue);
        expect(await this.token.methods['collateral()']()).to.be.bignumber.equal(firstTokenValue);
      });
    });
    ...
    context('batch', function () {
      ...
      it('collateral', async function () {
        expect(await this.token.methods['collateral(uint256)'](firstTokenId)).to.be.bignumber.equal(firstTokenValue);
        expect(await this.token.methods['collateral(uint256)'](secondTokenId)).to.be.bignumber.equal(secondTokenValue);
        expect(await this.token.methods['collateral()']()).to.be.bignumber.equal(
  ...
  context('after burn', function () {
    ...
    context('batch', function () {
      ...
      it('collateral', async function () {
        expect(await this.token.methods['collateral(uint256)'](firstTokenId)).to.be.bignumber.equal('0');
        expect(await this.token.methods['collateral(uint256)'](secondTokenId)).to.be.bignumber.equal('0');
        expect(await this.token.methods['collateral()']()).to.be.bignumber.equal('0');
      });
    });
  });

  ...
  for (const [mode, artifact] of Object.entries(MODES)) {
    describe(`vote with ${mode}`, function () {
      ...
      describe('getPastcollateral', function () {
        ...
        it('reverts if block number >= current block', async function () {
          const clock = await this.token.clock();
          await expectRevertCustomError(this.token.getPastcollateral(5e10), 'ERC5805FutureLookup', [5e10, clock]);
        });

        it('returns 0 if there are no checkpoints', async function () {
          expect(await this.token.getPastcollateral(0)).to.be.bignumber.equal('0');
        });
        ...
        it('returns the latest block if >= last checkpoint block', async function () {
          ...
          expect(await this.token.getPastcollateral(timepoint)).to.be.bignumber.equal(supply);
          expect(await this.token.getPastcollateral(timepoint + 1)).to.be.bignumber.equal(supply);
        });
        ...
        it('returns zero if < first checkpoint block', async function () {
          ...
          expect(await this.token.getPastcollateral(timepoint - 1)).to.be.bignumber.equal('0');
          expect(await this.token.getPastcollateral(timepoint + 1)).to.be.bignumber.equal(
            '10000000000000000000000000',
          );
        });
        ...
        it('generally returns the voting balance at the appropriate checkpoint', async function () {
          ...
          expect(await this.token.getPastcollateral(t1.timepoint - 1)).to.be.bignumber.equal('0');
          expect(await this.token.getPastcollateral(t1.timepoint)).to.be.bignumber.equal('10000000000000000000000000');
          expect(await this.token.getPastcollateral(t1.timepoint + 1)).to.be.bignumber.equal(
            '10000000000000000000000000',
          );
          expect(await this.token.getPastcollateral(t2.timepoint)).to.be.bignumber.equal('9999999999999999999999990');
          expect(await this.token.getPastcollateral(t2.timepoint + 1)).to.be.bignumber.equal(
            '9999999999999999999999990',
          );
          expect(await this.token.getPastcollateral(t3.timepoint)).to.be.bignumber.equal('9999999999999999999999980');
          expect(await this.token.getPastcollateral(t3.timepoint + 1)).to.be.bignumber.equal(
            '9999999999999999999999980',
          );
          expect(await this.token.getPastcollateral(t4.timepoint)).to.be.bignumber.equal('10000000000000000000000000');
          expect(await this.token.getPastcollateral(t4.timepoint + 1)).to.be.bignumber.equal(
            '10000000000000000000000000',
          );
        });
      });

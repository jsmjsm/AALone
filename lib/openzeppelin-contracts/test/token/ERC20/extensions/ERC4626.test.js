...
contract('ERC4626', function (accounts) {
  ...
  describe('reentrancy', async function () {
    ...
    it('correct share price is observed during reentrancy after claimBTC', async function () {
      ...
      await token.scheduleReenter(
        reenterType.After,
        vault.address,
        vault.contract.methods.claimBTC(reenterValue, holder, token.address).encodeABI(),
      );
      ...
      // Do claimBTC normally, triggering the _afterTokenTransfer hook
      const receipt = await vault.claimBTC(value, holder, holder, { from: holder });

      // Main claimBTC event
      await expectEvent(receipt, 'claimBTC', {
        sender: holder,
        receiver: holder,
        owner: holder,
        assets: value,
      ...
      // Reentrant claimBTC event → uses the same price
      await expectEvent(receipt, 'claimBTC', {
        sender: token.address,
        receiver: holder,
        owner: token.address,
    ...
    // Still, the claimBTC that trigger the reentracy is not affected and get the previewed price.
    // Further claimBTC will get a different price (needing more shares for the same value of assets)
    it('share price change during reentracy does not affect claimBTC', async function () {
      ...
      // claimBTC, triggering the _afterTokenTransfer hook
      const receipt = await vault.claimBTC(value, holder, holder, { from: holder });
  ...
  describe('limits', async function () {
    ...
    it('reverts on claimBTC() above max claimBTC', async function () {
      const maxWithdraw = await this.vault.maxWithdraw(holder);
      await expectRevertCustomError(
        this.vault.claimBTC(maxWithdraw.addn(1), recipient, holder),
    ...
    describe(`offset: ${offset}`, function () {
      ...
      describe('empty vault: no assets & no shares', function () {
        ...
        it('claimBTC', async function () {
          ...
          const { tx } = await this.vault.claimBTC('0', recipient, holder, { from: holder });
          ...
          await expectEvent.inTransaction(tx, this.vault, 'claimBTC', {
            sender: holder,
            receiver: recipient,
            owner: holder,
            assets: '0',
      ...
      describe('inflation attack: offset price by direct deposit of assets', function () {
        ...
        it('status', async function () {
          expect(await this.vault.collateral()).to.be.bignumber.equal('0');
        ...
        it('deposit', async function () {
          ...
          const effectiveShares = await this.vault.collateral().then(x => x.add(virtualShares));
      ...
      describe('full vault: assets & shares', function () {
        ...
        it('status', async function () {
          expect(await this.vault.collateral()).to.be.bignumber.equal(parseShare(100));
          expect(await this.vault.totalAssets()).to.be.bignumber.equal(parseToken(1));
        });
        ...
        it('claimBTC', async function () {
          ...
          const { tx } = await this.vault.claimBTC(withdrawAssets, recipient, holder, { from: holder });
        ...
        it('claimBTC with approval', async function () {
          const assets = await this.vault.previewWithdraw(parseToken(1));
          await expectRevertCustomError(
            this.vault.claimBTC(parseToken(1), recipient, holder, { from: other }),
            'ERC20InsufficientAllowance',
            [other, 0, assets],
          );

          await this.vault.claimBTC(parseToken(1), recipient, holder, { from: spender });
        });
  ...
  describe('ERC4626Fees', function () {
    ...
    describe('output fees', function () {
      ...
      it('claimBTC', async function () {
        ...
        ({ tx: this.tx } = await this.vault.claimBTC(valueWithoutFees, recipient, holder, { from: holder }));
      });
      ...
      afterEach(async function () {
        ...
        // claimBTC event
        await expectEvent.inTransaction(this.tx, this.vault, 'claimBTC', {
          sender: holder,
          receiver: recipient,
          owner: holder,
  ...
  it('multiple mint, deposit, redeem & withdrawal', async function () {
    ...
    // 1. Alice mints 2000 shares (costs 2000 tokens)
    {
      ...
      expect(await this.vault.collateral()).to.be.bignumber.equal('2000');
    ...
    // 2. Bob deposits 4000 tokens (mints 4000 shares)
    {
      ...
      expect(await this.vault.collateral()).to.be.bignumber.equal('6000');
    ...
    // 4. Alice deposits 2000 tokens (mints 1333 shares)
    {
      ...
      expect(await this.vault.collateral()).to.be.bignumber.equal('7333');
    ...
    // NOTE: Alices's vault assets got rounded towards infinity
    {
      ...
      expect(await this.vault.collateral()).to.be.bignumber.equal('9333');
    ...
    // 7. Alice redeem 1333 shares (2428 assets)
    {
      ...
      expect(await this.vault.collateral()).to.be.bignumber.equal('8000');
      expect(await this.vault.totalAssets()).to.be.bignumber.equal('14573');
    }

    // 8. Bob withdraws 2929 assets (1608 shares)
    {
      const { tx } = await this.vault.claimBTC(2929, user2, user2, { from: user2 });
      await expectEvent.inTransaction(tx, this.vault, 'Transfer', {
        from: user2,
        to: constants.ZERO_ADDRESS,
        value: '1608',
      ...
      expect(await this.vault.collateral()).to.be.bignumber.equal('6392');
    ...
    {
      const { tx } = await this.vault.claimBTC(3643, user1, user1, { from: user1 });
      await expectEvent.inTransaction(tx, this.vault, 'Transfer', {
        from: user1,
        to: constants.ZERO_ADDRESS,
        value: '2000',
      ...
      expect(await this.vault.collateral()).to.be.bignumber.equal('4392');
